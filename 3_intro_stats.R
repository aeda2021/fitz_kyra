...
# second, stats test
cor(mtcars$mpg, mtcars$wt, method = "pearson")  #-0.8677
...
#shows the 3 outliers clustered at the high end of the distribution on the qq plot and the histogram
...
#wt seems to be normally distributed in mtcars
...
cor.test(mtcars$mpg, mtcars$wt)  #correlation is -0.868, p-value is 1.294e-10

# Q: put your conclusion from this analysis in words
#Miles per gallon (mpg) and weight (wt) in the mtcars dataset show a statistically significant negative correlation, with 
#a correlation value of -0.868. This tells us that as the weight of a car increases, its mpg decreases.
...
#correlation looks for a relationship between data, but cannot predict values
...
#yes- each row is a one observation (a car make/model), and each column is one variable
...
#slope would be about -5
# Q: the intercept?
#intercept around 35-36
# Q: the df for the error?  Why?
#32-2=30, N-2 because 1 df used to predict the intercept, and 1 df used to predict the slope
...
#in R, we need to save/assign names to models, this allows us to specify what kind of outputs we want
#such as the summary of the linear regression or an anova table

# second, run stats test
# reminder that R reads the lm function as saying, 'fit a linear model, where we predict mpg is a function of car weight, 
#using variables from the mtcars data frame'
# you'll like yourself better in the future if you get in the practice of saving all model results 
#in a consistent name format
...
#Residuals vs. fitted- points should be evenly distributed- ours look fairly evenly distributed
#Normal QQ plot- points should fall along a straight line- ours do
#Scale-Location plot- looking if vertical spread of points varies from left to right, want to see an even spread,
#ours looks fairly even
#Residuals vs. leverage- checking if our points are in the upper or lower right (might be outliers), 
#we have a few points towards the right, but this seems okay given some car models 
...
#Yes they are about what we expected, this is likely because our data fits a linear model
...
# Q: Why might it be a good idea to name the slope and intercept terms like this before plotting them, 
#rather than just typing in the numerical values?
#To avoid any errors
...
# augment() creates an augmented version of the model results df that includes the fitted values 
#and 95% CI corresponding to each data point
...
#this 95% CI is for the mean, not for all our data points, so yes this is something we expect
...
#yes car weight can predict mpg (statistically significant lm with r-squared of 0.75)
# Q: can you interpret your intercept in a meaningful way? 
#A weight of 0 would yield an mpg of 37 in our model (as 37 is the y-intercept), so there is a limit to how far we can predict
#as cars do not actually weight 0
...
#check for dat within groups are normally distributed and have similar variance, looks good for our data
# Q: what is your guess about the answer to our anova question? be as specific as possible, eg, rank species in order
#Predict that irises of different species do have different sepal widths, expect setosa to have the highest, but may not see
#a significant difference between virginica and versicolor
# second, run model
...
#the normal qq plot is helpful, but the others don't seem to be
...
#the data seems to meet the assumptions
...
#the intercept estimate is the mean of the reference group (which is the species setosa here)
# Q: what do the t values mean, for both the estimate (reference level) and the other comparisons? 
#t-test comparing two distributions asking if they're different or not, intercept estimate is being compared to zero,
#tells us it's significantly different from zero
# Q: what does the F and its p value tell you?
#The F is large and the p-value is very small, which indicates the groups have different means

anova(iris_mod) # remember this is how we get anova-type results out of the model we ran
# Q: why are the df what they are? 
#for species- the df is n-1, so 3 species - 1 = 2, 
#for residuals is n-3 based on total number of data points, 150-3= 147, estimating 3 different means (1 for each species)

# Q: what do the two MS values tell you?
#The MS for Species is the group MS, which is the part of the total variance attributable to groups (species) having different means
#The MS for Residuals is the error MS, which is the part of the total variance that is the individuals varying within a group
...
versicolor_mean <- mean(versicolor$Sepal.Width) #2.77
virginica_mean <- mean(virginica$Sepal.Width) #2.974
setosa_mean <- mean(setosa$Sepal.Width) #3.428

#virginica-versicolor diff
virginica_mean-versicolor_mean  #yes diff of 0.204 is the same as Tukey result

#setosa-versicolor diff
setosa_mean-versicolor_mean #yes diff of 0.658 is the same as Tukey result

#setosa-virginica diff
setosa_mean-virginica_mean #yes diff of 0.454 is the same as Tukey result
...
#The F-statistic and overall p-value stay the same, the intercept estimate is different (it's the estimate of a different Species 
#mean now)

# fifth, plot model results back onto the data picture
# the tidyverse function 'augment; doesn't work on anova models, so we need to do this a clunkier way with the 
#base R function 'predict'
# getting the mean for each level and the associated se 
iris_mean_se = predict(iris_mod, se=TRUE, newdata=data.frame(Species=c("versicolor", "virginica", "setosa"))) 

# the newdata argument is how we tell R to predict for levels, not individual data points
