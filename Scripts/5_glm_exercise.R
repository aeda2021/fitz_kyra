...
#Small means show we would need negative counts for this to be normal, which isn't possible
# another way of stating this problem, which again ecologists frequently run into, is that if zero is one of your more frequent data values, 
#your data cannot be normal
# on the other hand, when the mean of the distribution (e.g., the mean number of plants per quadrat) is large enough, 
#the distribution moves to the right away from the Y axis
...
sheep_mod <- lm(fitness ~ body.size, data = soay)

hist(sheep_mod$residuals)
plot(sheep_mod)
...
#Our y variable is count data, and the plot looks like an exponential relationship
...
AIC(sheep_mod)
#230, the GLM has a lower AIC, showing GLM family poisson may be a better fit than the linear model

# Third, interpret results
# just like for lm, you can get either anova-type output or regression-type output for glm
# remember anova() is messed up wrt the order in which you enter your predictors mattering, but, 
#since we only have one predictor here it's okay
anova(soay_mod1)
...
# next, the dispersion parameter: this can be important for some families, but the poisson doesn't have a dispersion parameter, 
#so ignore for now
...
# last, the AIC
# as usual, we can use this to compare among models containing different predictor variables, so long as they are run on the 
#same dataset
# we can also use AIC to compare among models using different families (eg, poisson versus negative binomial), so long as the 
#linear predictor and the dataset is the same
# remember AIC is better for comparing among models than likelihood, because AIC penalizes for additional parameters
# lastly, there is the number of Fisher scoring iterations, but that is R's problem, not ours (it just has to do with how hard R 
#had to work to fit the model)
...
# this is different from what we are used to thinking about with linear fits, where y increases by a constant absolute amount for 
#each unit change in x (e.g., y=2x always changes by 2 per unit change in x, regardless of where you are on the graph)
...
# if you took a look at that output and wondered why the deviance residuals (remember this is the relevant type for glm) aren't equal to 
#(fitted - observed), that's because they aren't
...
# a key thing to recognize here is that the se should be symmetrical on the link scale, but when we back transform them they will be 
#asymmetrical on the original data scale
...
soay_mod1_aug_response = augment(soay_mod1, data=soay, type.predict="response", se_fit=T) # reminder of what we already did above, 
#to get the fitted line - but now we name the object different so that we don't get totally messed up
soay_mod1_aug_response
# we do get a .se.fit here, so why can't we use it?
# the reason is that, although this is indeed the se on the scale of the response, 1.96 * this value will not be equal to the 95% CI 
...
data(titanic_train)  # just fyi, the _train part is there because this is a 'training' data set assembled by someone for machine learning
#purposes; it's irrelevant to what we are doing
...
# you should always do this sort of thing at the start of an analysis to get a sense of what the data look like, and 
#whether there are outliers, or likely erroneous values
hist(titanic$Fare, breaks = 20)
#look at other variables of interest:
hist(titanic$Pclass)
...
# the data picture stage is also a good time to think about possible relationships among your variables and ask yourself whether you 
#really want to include all of them
...
# we should check for correlations among the variables, but that is easiest to do using variance inflation factors (vif) after 
#running the model
...
# we can get the standardized residuals with augment, and look at them with a histogram - so back in histogram of residuals territory, 
#but now without worrying about normality!
...
# "weight" is a ratio of relative model likelihoods, usually interpreted as the probability that the given model is actually the best of the 
#set of models you ran
# the parameters are listed in columns (intercept, Sex, Pclass), before the model summary stats  
# if a parameter is in the model it has the estimated coefficient value for continuous predictors, or a "+" for categorical predictors.  
#Parameters that weren't in that model have a blank space.
# note model.sel() is actually running AICc here instead of AIC
# AICc is a version of AIC that attempts to correct for bias with small sample size and converges to regular AIC as sample size increases. 
#It doesn't really have any downsides that I know of.
# Q: discuss with your partner what you conclude about the results of the model selection
#the full model performs the best compared to the other models

# NOW for the HAIRY part of logistic regression: INTERPRETING YOUR MODEL PARAMETERS
# remember the parameter values R returns are on the scale of the link function, which is the log-odds scale - that is the log of the odds 
#ratio, log(p/(1-p))
...
# thus, an intercept of 0 would mean a 50/50 chance of 'success' (for us, survival - so it actually makes sense to call it 
#success for once!) when factors such as class and sex are not taken into account 
...
# well, it would be the log odds of survival for females (reference level sex=0) who are Pclass=0...  which is sort of nonsense, but 
#let's figure it out anyway
odds = exp(3.2946)	
...
# females in class 1
odds = exp(3.2946 + 1*(-0.9606))	# remember we do the addition in terms of log odds, 
#since that's how the coefficients are reported, and then convert the answer to odds 
...
odds_female2 <- exp(3.2946 + 2*(-0.9606))
odds_female2 #survival odds of about 4:1
odds_female2 / (1 + odds_female2) #survival probability of 0.80

# Q: let's up our game: calculate the survival odds and probability of survival for males in class 3

#start with the estimate for the intercept, and adjust it for males, 3.2946-2.6434, then adjust for class 3 (3*0.9696)
odds_male <- exp(3.2946-2.6434-(3*0.9606))
odds_male
odds_male / (1 + odds_male) #survival probability of 0.097


## Fourth, plot results back onto a data picture
# for logistic regression, people usually show the data and the curved model fit, with the continuous variable on the x and 
#the probability on y
