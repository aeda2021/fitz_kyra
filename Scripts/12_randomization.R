...
#The resampled distribution is more clustered than the original distribution and has less even of a spread. It looks like not all individual pennies
#are used in the resampling.

# Q: do the two distributions have the same mean? write code below to find out

mean(pennies_resample$year) #1996.4
mean(pennies_sample$year) #1994.8 

#The two distributions do not have the same mean, but the means are fairly close to each other.

# Q: what would happen if you sampled without replacement?

#The resampled distribution would be the same as the original distribution, as each penny can only be sampled once.
...
penny_means <- pennies_resample %>%
  group_by(replicate) %>%
  summarise(penny_mean=(mean(year)))

hist(penny_means$penny_mean)
...
#As we are running 100 replicates, each replicate can potentially be sampling different individuals at different frequencies, which can lead 
#to a pretty even spread of means. 
...
#I predict the distribution will be less even with less replicates.

pennies_resample_50 <- pennies_sample %>% 
  rep_sample_n(size = 50, replace = TRUE, reps = 50)

penny_means_50 <- pennies_resample_50 %>%
  group_by(replicate) %>%
  summarise(penny_mean=(mean(year)))

hist(penny_means_50$penny_mean)

#Yes, with less replicates, the distribution is less even and is more clustered.
...
pennies_resample_lowsample <- pennies_sample %>% 
  rep_sample_n(size = 10, replace = TRUE, reps = 100)

penny_means_lowsample <- pennies_resample_lowsample %>%
  group_by(replicate) %>%
  summarise(penny_mean=(mean(year)))

hist(penny_means_lowsample$penny_mean)

#With a low sample size, my distribution has the same general shape as the original sample size but has a higher frequency around the middle
#of the distribution.

# now let's get the bootstrapped CI

pennies_resample_means <- penny_means

CI <- pennies_resample_means %>%
  summarise(
    l = quantile(penny_mean, prob = 0.025), 
    u = quantile(penny_mean, prob = 0.975)
  )
...
ggplot(data = pennies_resample_means, aes(x=penny_mean)) +
...
penny_medians <- pennies_resample %>%
  group_by(replicate) %>%
  summarise(penny_median=(median(year)))

Median95 <- penny_medians %>%
  summarise(
    l = quantile(penny_median, prob = 0.025), 
    u = quantile(penny_median, prob = 0.975)
  )

ggplot(data = penny_medians, aes(x=penny_median)) +
  geom_histogram(binwidth = 1, color = "white") +
  geom_vline(xintercept = CI$l, color = "green") +
  geom_vline(xintercept = CI$u, color = "green")
...
#No these data are not tidy. They should be tidy to allow for easier data manipulation.
...
#If replacement is set to false, then each individual will only be sampled once, which is necessary for a permutation. 
...
#The column sums are maintained because we are reshuffling within the column and are only sampling each individual once.

rowSums(bee_prefs)
rowSums(bee_prefs_perm)
# Q: are row sums maintained?  should they be?

#The row sums are not maintained because we permuted by columns.

# Q: write code below to do a permutation similar to that above,
# but now permute the rows instead of the columns

for (i in 1:15) {
  bee_prefs_perm[i,] = sample(bee_prefs[i,], replace=F)	
}
...
#Row sums are now the same, which makes sense because we are permuting by rows this time.
...
#The function randomly draws a value (a number of visits) from the visits data, and it randomly draws a value (number of pollen grains) from the 
#pollen dataset. 

# Q: why did we use size=visit_draw?

#We need to draw the same size as the number of visits we drew, so that the two match up when we multiply them to get the amount of pollination.
...
hist(rgamma(173, shape=shape, scale=scale))
...
#The gamma distribution is the best fit, as it has the lowest AIC.
...
# shape parameter = (mean/sd)^2
# scale parameter = sd^2 / mean
sd_pollen <- sd(pollen$consp_grains)
sd_pollen
mean_pollen <- mean(pollen$consp_grains)
mean_pollen

shape1 <- (mean_pollen/sd_pollen)^2
shape1
scale1 <- sd_pollen^2 / mean_pollen
scale1

for (i in 1:10000) {
  visit_draw1 <- rgamma(1, scale = scale, shape = shape) 
  pollen_draws1 <- rgamma(visit_draw1, scale = scale1, shape = shape1)
...
#R returned a distribution of means after bootstrapping with 100 replicates.
...
bee_mass_bootstrap <- bee_mass %>%
  specify(response = dry_mass_mg) %>%
  generate(reps = 10000, type = "bootstrap") %>%
  calculate(stat = "mean")

bee_mass_CI <- bee_mass_bootstrap %>%
  summarise(
    l = quantile(stat, prob = 0.025), 
    u = quantile(stat, prob = 0.975)
  )

bee_mass_CI #27.3-32.1
   
# compare bootstrap mean to the data mean
mean(bee_mass$dry_mass_mg) #29.7, which is right in the middle of the CI
...
#Differences in means are in the stat column, which is the difference in the queen mean and the worker mean.

# Q: how will you use these numbers? what are we trying to test here?

#It is randomizing what weight is assigned to which individual. We are running this permutation to create a null model where bee mass does not 
#depend on the caste (queen vs. worker bee).
...
#The value is close to zero, which is what I would expect given that we are trying to permute the data so that bee mass does not depend on caste,
#meaning the difference in means should be around zero.
...
#The permutation test would show that mass does differ by caste in comparison to our null model.
...
#Our bootstrapped results are showing the queen and worker bees together in the same distribution, which yields a distribution of means for the 
#whole population that does not encompass the mean for the queen or worker bees separated out/on their own.
