...
Marginal probability is the probability of something irrespective of everything else. In our model, this means the marginal probability of a parameter is the probability of that parameter irrespective of everything else. So, the marginal probability of beta 1 is the probability of beta 1 irrespective of anything else. 
...
datList.crabs <- list(
   y = crabs$burrows, #observations
  N = length(crabs$burrows),  #number of observations
  x1 = crabs$salinity # salinity data
)
...
plotPost(fm.crabs.dat$alpha, main = "Crab Burrow Density at Mean Salinity (Log Scale)")
plotPost(fm.crabs.dat$b1, main = "Effect of Salinity on Crab Burrow Density (Log Scale)")
plotPost(exp(fm.crabs.dat$alpha), main = "Crab Burrow Density at Mean Salinity")
```
...
*Answer:*
*The expected number of burrows is 9.858, and it's confidence interval is 8.53-11.3. 
*The intercept is the number of crab burrows at mean salinity. If we hadn't centered the salinity measurements, it would be the number of crab burrows at a salinity of zero. This would not likely be helpful for us if these are crabs that live in marine environments where salinity would not be zero.
*The mean effect of salinity is 0.0378 (on the log scale) and the confidence interval is 0.023-0.0518. This means there is a positive effect of salinity on the density of crab burrows, with the number of crab burrows increasing by a factor of log(0.0378) for each unit of increase in salinity.
*The probability that salinity has no effect is approximately zero because the posterior distribution of the effect of salinity only includes values greater than 0.01. The models shows that salinity has at least some effect on crab burrow density.
...
for(i in 1:N){
  y[i] ~ dbern(p[i])
  logit(p[i]) <- b[site[i]] + b1*x1[i] + b2*x2[i] + b3*x3[i]
}
## Random intercepts:# Now we index across site-level means- to declare each site-level intercept
for(j in 1:n.sites){
#each site-level intercept is the realization of a normal process:
  b[j] ~ dnorm(b0, 1/sig.site^2)
}

#Priors
#Model is coded as logit(p), so take the inverse of the priors described above:
b0 ~ dnorm(0, 0.5)
b1 ~ dnorm(0, 0.2)
b2 ~ dnorm(0, 0.2)
b3 ~ dnorm(0, 0.2)
...
datList.ticks <- list(
  N = length(ticks$forest), # Number observations
...
fm.ticks <- jags(
  data = datList.ticks,
  model.file = tick.mod,
  parameters.to.save = c('b0', 'b1', 'b2', 'b3', 'sig.site'),
  n.chains = 3,
  n.iter = 20000,
  n.burnin = 5000,
  n.thin = 3
...
plotPost(fm.ticks.dat$b0, main = "EC Occurrence at Mean Forest Area, Edge, and Type (Intercept)")
plotPost(plogis(fm.ticks.dat$b1), main = 'Effect of Forested Area on EC Occurrence')
plotPost(plogis(fm.ticks.dat$b2), main = 'Effect of Forest Edge on EC Occurrence')
plotPost(plogis(fm.ticks.dat$b3), main = 'Effect of Evergreen Proportion on EC Occurrence')
plotPost(plogis(fm.ticks.dat$sig.site), main = 'Among-Site Variance in EC Occurrence')
plotPost(plogis(fm.ticks.dat$b0), main = 'Probablity of Presence of EC at Mean Forest Area, Edge, and Type')
```
...
*Answer:*
The mean probability of a plot testing positive for EC was 0.279, with a 95% confidence interval of 0.141-0.424. The it appears that forest area and edge have an effect on EC occupancy. As area and edge increase, there is a lower probability of a plot testing positive for EC (a lower EC occupancy). Forest type (evergreen proportion) does not appear to contribute to lower EC occupancy, and it does not seem like forest type has an effect on EC occupancy.
