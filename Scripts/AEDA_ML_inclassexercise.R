...
#We will fit this using the "train" function from caret, which can fit traditional linear models 
#as well as models incorporating machine learning
apriori_formula <- y ~ AdultBodyMass_g + AgeatFirstBirth_d + BasalMetRate_mLO2hr + 
  GestationLen_d + LittersPerYear + MaxLongevity_m
...
#Basically, we fit a linear model with all of the possible variables, then impose a "regularization" penalty 
#on some coefficients
...
#Machine Learning Model II: gaussian process model
