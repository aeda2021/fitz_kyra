...
ggplot(koalas, aes(x=northing, y=easting, color=as.factor(presence))) +
	geom_point()
...
No, I do not see any signs of colinearity that concern me. The magnitudes are all low and well below 0.5.
...
Yes, I see some positive spatial auto-correlation at very low distances, from about 0 to 700 km. This is intuitive due to the potential for auto-correlation within a koala's home range.
...
The QQ plot looks good; the black dots follow the red line closely. The residual vs. predicted plots also look good, they are equally distributed, and there are not any outliers.
...
There is still spatial auto-correlation at distances less than 700 km. This likely relates to how there are sub-sites within sites, and how koalas have home ranges that are larger than the sub-sites and may overlap with each other.
...
```{r glmmfit}
mod_glmm <- glmmTMB(presence ~ pprim_ssite + psec_ssite + pdens_1km + (1|site), family = 'binomial', data = koalas_st)
```
...
correlog_glmm <- spline.correlog(x = koalas$easting,
                            y = koalas$northing,
                            z = resid(mod_glmm), 
                            xmax = 10000)
plot(correlog_glmm)
...
The QQ plot looks good, and the points follow the red line nicely. The residuals vs. predicted plot has points spread out evenly, and looks appropriate. The correlogram looks better than before with only a glm, but there is now some negative autocorrelation at low distances. Each residuals vs. explanatory plots also appear to be evenly distributed and look okay. 
...
```{r}
mod_glmm2 <- glmmTMB(presence ~ pprim_ssite + psec_ssite + (1|site), family = 'binomial', data = koalas_st)
```
...
The model with only site-scale variables in it appears to be the parsimonious model here according to AIC. This tells us that koala spatial distributions are most influenced by the percent of trees in each subsite that are primary tree species and the percent of trees in each subsite that are secondary tree species. They do not appear to be very influenced by the density of habitat patches.
